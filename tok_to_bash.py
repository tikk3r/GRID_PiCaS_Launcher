...
def export_tok_keys(cfgfile='tokvar.cfg',token=os.environ['TOKEN']):
    ...
    for key in tokvar:
        if isinstance(tokvar[key],basestring):
            picas_val=str(get_token_field(token,key,db,un,pwd))
            os.environ[tokvar[key].split('$')[1]]=picas_val
        elif key=='_attachments':
            t_type=get_token_field(token,'type',db,un,pwd)
