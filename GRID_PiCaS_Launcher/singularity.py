...
def convert_shub_to_http(
    ...
    shub_http_url = "{0}/{1}/{2}/{3}".format(
        repo_base_url, shub_user, shub_collection, shub_image
    )
    if shub_commit:
...
def check_if_http_sif(http_url):
    ...
    :param http_url: the URL which needs to be checked
    :type http_url: str"""
    r = urlopen(http_url)
...
def parse_singularity_link(simg_url, simg_commit=None):
    ...
    if 'http' in simg_url.split("://")[0]:
        return download_simg_from_http(simg_url)
...
def download_simg_from_gsiftp(simg_link):
    ...
    _dl = subprocess.Popen(
        ["globus-url-copy", simg_link, img_name],
...
def download_simg_from_http(simg_link, sif_name=None):
    """Downloads an image from a http link and 
    saves it to a .sif image (can be set to a custom image name)"""
    if not sif_name:
...
def get_newest_file():
    list_of_files = glob.glob(os.getcwd() + "/*.sif") + glob.glob(
        os.getcwd() + "/*.simg"
    )
...
def get_image_path(subprocess_popen):
    ...
    if os.path.exists(img_path):
        ...
        raise RuntimeError(
            "Tried to download image to {0} but now it isn't there!".format(img_path)
        )
...
def pull_image_from_shub(shub_link, commit=None):
    ...
    if not commit:
        _pull = subprocess.Popen(
            ["singularity", "pull", shub_link],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
    else:
        _pull = subprocess.Popen(
            ["singularity", "pull", "{0}@{1}".format(shub_link, commit)],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
...
def put_variables_in_env(json_payload):
    ...
    :param json_payload: The json object encoding the token as stored in CouchDB
