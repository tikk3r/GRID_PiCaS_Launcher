...
function dl_generic(){
   ...
   if [[ ! -z $( cat $1 | grep psnc )  ]]; then
     sed 's?srm://lta-head.lofar.psnc.pl:8443?gsiftp://gridftp.lofar.psnc.pl:2811?g' $1 | xargs -I{} globus-url-copy -rst  -st 30 -v {} $PWD/Input/ || { echo 'downloading failed' ; exit 21; }
   fi
   ...
   echo "Download Done! We're back in ${RUNDIR}"
...
if [ ! -n "$(type -t download_files )"   ] && [ ! "$(type -t download_files )" = function   ]; then
    ...
    function download_files(){
        ...
        srmlist=$1
        if [ -z "$srmlist" ]; then 
            echo "No variable with srmlist set" 
            return;
        fi
